...
def recommend_by_user_id(user_id):
    ...
    if current_user.rate_set.count() == 0:
        ...
        return movie_list
    # 选取评分最多的10个用户
    users_rate = Rate.objects.values('user').annotate(mark_num=Count('user')).order_by('-mark_num')[:10]
...
def similarity(movie1_id, movie2_id):
    ...
    # 两者的交集
    common = Rate.objects.filter(user_id__in=Subquery(movie1_set.values('user_id')), movie=movie2_id).values(
        'user_id').count()
    # 没有人给当前电影打分
    ...
    for un_watched_movie in un_watched:
        for watched_movie in watched:
            if un_watched_movie not in names:
                ...
                distances.append(
                    (similarity(un_watched_movie.id, watched_movie[0]) * watched_movie[1], un_watched_movie))
